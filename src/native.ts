...
export const native = {
  setLogOptions: (opts: { database?: string; client?: string; color?: boolean }) => call<{}>('WmSetLogOptions', opts),
  openContainer: (opts: { dialect: string; address: string }) => call<{ handle: number }>('WmOpenContainer', opts),
  containerGetFirstDevice: (handle: number) => call<{ handle: number }>('WmContainerGetFirstDevice', { handle }),
  containerGetAllDevices: (handle: number) => call<{ handles: number[] }>('WmContainerGetAllDevices', { handle }),
  containerGetDevice: (handle: number, jid: string) => call<{ handle: number; found: boolean }>('WmContainerGetDevice', { handle, jid }),
  ...
  clientSendPresence: (client: number, state: string) => call<{}>('WmClientSendPresence', { client, state }),
  clientSubscribePresence: (client: number, jid: string) => call<{}>('WmClientSubscribePresence', { client, jid }),
  clientSendChatPresence: (client: number, jid: string, state: string, media: string) => call<{}>('WmClientSendChatPresence', { client, jid, state, media }),
  clientUpload: (client: number, dataB64: string, type: string) => call<any>('WmClientUpload', { client, data: dataB64, type }),
  clientDownloadByPath: (client: number, p: { direct_path: string; enc_sha256: string; sha256: string; media_key: string; file_length: number; type: string; mms_type?: string }) =>
    call<{ data: string }>('WmClientDownloadByPath', { client, direct_path: p.direct_path, enc_sha256: p.enc_sha256, sha256: p.sha256, media_key: p.media_key, file_length: p.file_length, type: p.type, mms_type: p.mms_type ?? '' }),
  clientGetGroupInviteLink: (client: number, jid: string, reset?: boolean) => call<{ link: string }>('WmClientGetGroupInviteLink', { client, jid, reset: !!reset }),
  // Pairing code convenience wrapper (calls Client.PairPhone via WmClientCall)
  clientPairPhone: (client: number, phone: string, showPushNotification: boolean, clientType: number, clientDisplayName: string) =>
    call<string>('WmClientCall', { client, method: 'PairPhone', args: [phone, !!showPushNotification, clientType, clientDisplayName] }),
  clientStartEvents: (client: number) => call<{ handle: number }>('WmClientStartEvents', { client }),
  eventNext: (handle: number, timeoutMs: number) => call<any>('WmEventNext', { handle, timeoutMs }),
  clientIsLoggedIn: (client: number) => call<{ isLoggedIn: boolean }>('WmClientIsLoggedIn', { client }),
  clientDisconnect: (client: number) => call<{}>('WmClientDisconnect', { client }),
  clientWaitForConnection: (client: number, timeoutMs: number) => call<{ ok: boolean }>('WmClientWaitForConnection', { client, timeoutMs }),
  clientCall: (client: number, method: string, args: any) => call<any>('WmClientCall', { client, method, args }),
